...
[config.yaml]          [schema/*.yaml]       [relations.yaml（任意）]
      ...
      └──────────┬───────────┘                        │
         ...
         （一意性・参照整合性チェック）
                 │
         [クロステーブルチェック]
         （ユーザー定義のテーブル横断SQLチェック）
                 │
         [基本統計量算出]
         （共通統計: 一括SQL / 数値型追加統計: 列ごと個別SQL）
                 │
         [Parquetエクスポート（任意）]
         （テーブル＋リレーション＋クロスチェック結果でゲート）
                 │
         [HTMLレポート生成]
```
...
**`tval run [--config PATH] [--export] [--quiet] [--dry-run]`**

| オプション | 説明 |
|---|---|
| `--config` | `config.yaml`のパスを明示指定する |
| `--export` | バリデーション全成功時にParquetファイルを書き出す |
| `--quiet`, `-q` | エラー以外のログ出力を抑制する |
| `--dry-run` | 設定・スキーマのバリデーションのみ行い、データロードは実行しない |
| `--version` | バージョン番号を表示して終了する（`tval`サブコマンド直下） |
...
### クロステーブルチェック仕様（`cross_checks`）

`relations.yaml`内に`cross_checks`セクションを追加することで、複数テーブルにまたがるユーザー定義SQLチェックを実行できる。
...
cross_checks:
  - name: order_amount_vs_details_total
...
| フィールド | 型 | 必須 | デフォルト | 説明 |
|---|---|---|---|---|
| `name` | string | ✅ | - | チェック名（レポート表示用） |
| `tables` | string[] | ✅ | - | 参照テーブル名リスト（最低2つ必須） |
| `query` | string | ✅ | - | スカラー整数を返すSQLクエリ |
| `expect_zero` | boolean | ❌ | `true` | `true`: 結果0でOK、`false`: 結果>0でOK |

**注意事項**: クロスチェックのクエリはread_only接続で実行される。参照テーブルのいずれかにロードエラーまたはチェック失敗がある場合は`SKIPPED`。`tables`が2テーブル未満の場合は`ValidationError`。
...
| `CrossCheckDef` | `name`, `tables`, `query`, `expect_zero`(=True) | クロステーブルチェック定義。`tables`は最低2つ必須 |
| `RelationsConfig` | `relations`(=[]), `cross_checks`(=[]) | relations.yamlトップレベル |
...
**公開関数**:
- `load_relations(path)` → `RelationsConfig`（`from`→`from_`リネーム処理含む）
- `validate_relation_refs(relations, table_defs)` → `None`（未定義テーブル/カラムで`ValueError`）
- `validate_cross_check_refs(cross_checks, table_defs)` → `None`（未定義テーブルで`ValueError`）
- `run_relation_checks(conn, relations, all_load_errors, check_failed_tables=None)` → `list[CheckResult]`
- `run_cross_checks(conn, cross_checks, all_load_errors, check_failed_tables=None)` → `list[CheckResult]`

ロードエラーまたはチェック失敗があるテーブルを含むリレーション/クロスチェックは全チェック`SKIPPED`。一意性チェックは`GROUP BY ... HAVING COUNT(*) > 1`、参照整合性チェックは`LEFT JOIN ... WHERE ... IS NULL`で実装。
...
**公開関数**:
- `generate_report(table_reports, output_path, db_path, executed_at, relation_check_results=None, cross_check_results=None)` → `None`
...
**公開関数**: `run(config_path=None, export=False, dry_run=False)` → `None`
...
- `_log_dry_run_summary(ordered_defs, relations, cross_checks)` → `None`
- `_validate_output_dirs(db_path, output_path)` → `None`（書き込み不可時`SystemExit(1)`）
- `_connect_duckdb(db_path, read_only=False)` → `DuckDBPyConnection`（接続失敗時`SystemExit(1)`）

**実行フロー**:
1. config探索 → `ProjectConfig`でバリデーション
2. パス解決（config.yaml親ディレクトリ基準）
3. スキーマYAML読み込み → リレーション・クロスチェック読み込み（任意）→ 参照バリデーション
4. DAGロード順決定
5. `--dry-run`時: サマリーをログに出力して終了
6. DuckDB接続（既存ファイル削除・再作成）→ テーブル作成・データロード（read/write接続）
7. チェック・プロファイリング・リレーション検証・クロスチェック実行（read-only接続）
8. エクスポート（`--export`時、全テーブル+リレーション+クロスチェックOKの場合のみ。NGがあれば全テーブル`SKIPPED`）
9. HTMLレポート生成
...
| DuckDB接続失敗 | `SystemExit(1)` |
| output dir 書き込み不可 | `SystemExit(1)` |
...
| クロスチェックSQL実行エラー | `CheckResult(status=CheckStatus.ERROR)` | エラー内容をmessageに記録 |
| ロードエラー/チェック失敗のあるテーブルを含むクロスチェック | `CheckResult(status=CheckStatus.SKIPPED)` | スキップ理由をmessageに記録 |
...
[クロステーブルチェックセクション（cross_checks定義時のみ）]
  - クロスチェックサマリー（総数 / OK / NG / SKIPPED）
  - 各クロスチェック結果
```
...
| cross_checksクエリのSQL任意実行 | read_only接続で実行することでデータ破壊を防止。内部ツールとして許容 |
